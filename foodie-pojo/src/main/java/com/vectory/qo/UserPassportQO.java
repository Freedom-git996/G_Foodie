...
public class UserPassportQO implements Serializable {
    ...
    @NotBlank(message = "用户名不能为空", groups = {ValidatorGroup.RegisterBean.class, ValidatorGroup.LoginBean.class})
    @Length(min = 4, message = "用户名最小长度为4")
    @ApiModelProperty(value="用户名", name="username", example="Jack", required = true)
    private String username;

    @NotBlank(message = "密码不能为空", groups = {ValidatorGroup.RegisterBean.class, ValidatorGroup.LoginBean.class})
    @Length(min = 6, max = 15, message = "密码最小长度为6，最大为15")
    @ApiModelProperty(value="密码", name="password")
    private String password;

    @NotBlank(message = "确认密码不能为空", groups = {ValidatorGroup.RegisterBean.class})
